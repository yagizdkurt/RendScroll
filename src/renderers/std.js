/* Standard ("STD") block renderer.
   ...
   A leading "_" ("### _STD:") places the card in the right column; without it
   the card stays on the left. It never fetches files, never touches the
...
// True only for a "### STD:" / "### _STD:" heading (colon form).
function isStdHead(h) {
  return /^_?std\s*:/.test(stdLower(h.textContent).trim());
}
...
function normalizeStdMarkdown(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    if (/^###\s+_?\s*std\s*:/i.test(line)) { inStd = true; out.push(line); continue; }
    if (/^#{1,3} /.test(line)) { inStd = false; out.push(line); continue; }
    if (inStd && /^image\s*:/i.test(line.trim())) {
...
function enhanceStdSections(root) {
  ...
  heads.forEach((head) => {
    ...
    // A leading "_" ("### _STD:") places the card in the right column.
    const right = head.textContent.trim().startsWith("_");

    const card = document.createElement("div");
    card.className = right ? "std-card std-right" : "std-card";

    // Optional label after the colon ("### STD: Varış" -> "Varış" title).
    const headEls = [];
    const name = head.textContent.trim().replace(/^_?\s*std\s*:\s*/i, "").trim();
    if (name) {
